/**
 * Skeleton Components Index
 * 
 * Centralized exports for all skeleton/placeholder components
 */

export { default as MetricCardSkeleton } from "./MetricCardSkeleton";
export type { MetricCardSkeletonProps } from "./MetricCardSkeleton";

export { default as StatListSkeleton } from "./StatListSkeleton";
export type { StatListSkeletonProps } from "./StatListSkeleton";

export { default as GraphSkeleton, GraphEmptyState, DEFAULT_MIN_POINTS_TO_RENDER } from "./GraphSkeleton";
export type { GraphSkeletonProps } from "./GraphSkeleton";

export { default as DashboardPlaceholder } from "./DashboardPlaceholder";
export type { DashboardPlaceholderProps } from "./DashboardPlaceholder";

export { default as TableSkeleton } from "./TableSkeleton";
export type { TableSkeletonProps } from "./TableSkeleton";

export { default as CardListSkeleton } from "./CardListSkeleton";
export type { CardListSkeletonProps } from "./CardListSkeleton";

export { default as FormSkeleton } from "./FormSkeleton";
export type { FormSkeletonProps } from "./FormSkeleton";

export { default as PageHeaderSkeleton } from "./PageHeaderSkeleton";
export type { PageHeaderSkeletonProps } from "./PageHeaderSkeleton";
