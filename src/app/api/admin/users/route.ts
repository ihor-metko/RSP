...
/**
 * GET /api/admin/users
 * Unified endpoint for fetching users with filtering and pagination (root admin only)
 * 
 * Query parameters:
 * - simple: Return simple user list for autocomplete (default: false)
 * - q: Search query (for simple mode, alias for search)
 ...
 * - limit: Number of results per page (10, 25, 50, 100, default: 25)
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (simpleMode) {
      // Simple mode for autocomplete/search
    ...
    // Full mode with pagination and filtering
    // Pagination
    const page = Math.max(1, parseInt(searchParams.get("page") || "1"));
    const limit = Math.min(100, Math.max(1, parseInt(searchParams.get("limit") || searchParams.get("pageSize") || "25")));
