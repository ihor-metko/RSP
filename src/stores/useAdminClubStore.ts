 ...
 * Stores admin-specific club data with full operational details
 */
interface AdminClubState {
  // State
  clubs: AdminClubWithCounts[];
  clubsById: Record<string, AdminClubDetail>;
  currentClub: AdminClubDetail | null;
  ...
  _inflightFetchClubById: Record<string, Promise<AdminClubDetail>> | null;

  // Actions
  setClubs: (clubs: AdminClubWithCounts[]) => void;
  setCurrentClub: (club: AdminClubDetail | null) => void;
  ...
  ensureClubById: (id: string, options?: { force?: boolean }) => Promise<AdminClubDetail>;
  invalidateClubs: () => void;
  
  createClub: (payload: CreateClubPayload) => Promise<AdminClubDetail>;
  updateClub: (id: string, payload: UpdateClubPayload) => Promise<AdminClubDetail>;
  deleteClub: (id: string) => Promise<void>;

  // Selectors
  getClubById: (id: string) => AdminClubWithCounts | undefined;
  isClubSelected: (id: string) => boolean;
}

/**
 * Zustand store for managing admin club data
 * Uses /api/admin/clubs endpoints
 */
export const useAdminClubStore = create<AdminClubState>((set, get) => ({
  ...
  ensureClubById: async (id: string, options = {}) => {
    ...
    // Create new inflight request
    const inflightPromise = (async (): Promise<AdminClubDetail> => {
      set({ loadingClubs: true, clubsError: null });
      try {
        const response = await fetch(`/api/admin/clubs/${id}`);
        ...
        const club: AdminClubDetail = await response.json();
