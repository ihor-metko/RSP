...
describe("Price Rules API", () => {
  ...
  describe("GET /api/courts/:courtId/price-rules", () => {
    it("should return price rules for a court", async () => {
      ...
      expect(data.rules[0].ruleType).toBe("SPECIFIC_DAY");
      expect(data.rules[0].dayOfWeek).toBe(1);
      expect(data.rules[1].ruleType).toBe("SPECIFIC_DATE");
  ...
  describe("POST /api/courts/:courtId/price-rules", () => {
    ...
    it("should create a weekly price rule for root admin", async () => {
      ...
      expect(data.ruleType).toBe("SPECIFIC_DAY");
    ...
    it("should create a WEEKENDS rule for root admin", async () => {
      mockAuth.mockResolvedValue({
        user: { id: "admin-123", isRoot: true },
      });
    ...
    it("should create an ALL_DAYS rule for root admin", async () => {
      mockAuth.mockResolvedValue({
        user: { id: "admin-123", isRoot: true },
      });
    ...
    it("should create a HOLIDAY rule for root admin", async () => {
      mockAuth.mockResolvedValue({
        user: { id: "admin-123", isRoot: true },
      });
